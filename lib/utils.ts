...
const TRADING_VIEW_SUFFIX_MAP: Record<string, string> = {
  '.NS': 'NSE',
  '.BO': 'BSE',
  '.L': 'LSE',
  '.HK': 'HKEX',
...
export const toTradingViewSymbol = (rawSymbol: string) => {
  if (!rawSymbol) return '';
  ...
  const entry = Object.entries(TRADING_VIEW_SUFFIX_MAP).find(([suffix]) =>
  ...
  const [suffix, exchange] = entry;
  const base = cleaned.slice(0, -suffix.length);

  return base ? `${exchange}:${base}` : cleaned;
};